(() => {
  ...
  async function sendToGoogleSheet(action = 'attempt') {
    const endpoint = String(window.HISTORYVERSE_DATA_URL || '').trim();
    if (!endpoint || !state.result || !state.attemptKey) return false;
  ...
  function submitExam(auto = false) {
    ...
    saveAttempt();
    void sendToGoogleSheet('attempt');
    showResults();
  }
  ...
  async function saveFeedback() {
    ...
    const sent = await sendToGoogleSheet('reflection');
    els.feedbackStatus.textContent = sent
      ? 'Refleksi dihantar kepada cikgu dan disimpan. ✓'
      : 'Refleksi disimpan pada peranti ini. Sambungan Google Sheet belum diaktifkan.';
  }
